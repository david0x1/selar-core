...
import {SelarWrapper} from "./SelarWrapper.sol";
import {ISelarPair} from "../interfaces/ISelarPair.sol";
import {ISelarFactory} from "../interfaces/ISelarFactory.sol";

contract SelarPair is UniswapV2Pair, SelarWrapper {
    ...
    modifier onlyOperator() {
        require(msg.sender == ISelarFactory(factory).operator());
        _;
    }
    ...
    modifier onlyReserved(address token) {
        require(
            ...
            "Selarswap: NOT A RESERVED TOKEN"
        );
        _;
    }
    ...
    modifier keepBalance(address token) {
        ...
        _;
        require(_balanceOf(token) == prevBal, "Selarswap: BALANCE CHANGED");
    }
    ...
    function updateRegistry() public {
        _registry = ISelarFactory(factory).registry();
    }
