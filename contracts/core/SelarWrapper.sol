 ...
 * @author yapeswap
 * @notice
 *  SelarWrapper is a fork of Yearn Finance's BaseWrapper not to store the token address as
 *  the state variable.
 *
 */
abstract contract SelarWrapper {
        ...
        for (uint256 id = 0; id < vaults.length; id++) {
            ...
            if (availableShares > 0) {
                ...
                if (amount != WITHDRAW_EVERYTHING) {
                    ...
                    .sub(withdrawn).mul(10**uint256(vaults[id].decimals())).div( // NOTE: Changes every iteration
